 ...
 * Descr:   待付款订单的显示碎片
 */
public class ObligationOrderFragment extends Fragment {
     ...
     * @return
     */
    public static ObligationOrderFragment newInstance(){
        ObligationOrderFragment fragment = new ObligationOrderFragment();
    ...
    private void geOrderList(final int page) {
        ...
        AsyncHttpPostFormData.addFormData("condition", "wait_pay");
        okHttpClient.post("/v1/order/list", AsyncHttpPostFormData, new AsyncResponseHandler() {
            ...
            public void onSuccess(AsyncHttpResponse response) {
                Log.i(TAG, "onSuccess:待付款" + response.toString());
                Type type = new TypeToken<RspModel<List<OrderInfo>>>() {
                }.getType();
                List<OrderInfo> order = getAnalysis(response, type, "待付款");
