...
public class MultiLineRadioGroup extends ViewGroup implements OnClickListener {
    ...
    private List<CommentTag> childValues;
    ...
    public MultiLineRadioGroup(Context context, AttributeSet attrs,
        ...
        childValues = new ArrayList<CommentTag>();
...
//        }
        if (childCount > 0) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (hasValues && i < childValues.size()) {
                    cb.setText(childValues.get(i).getLabel_detail());
                } else {
                    childValues.add(childValues.get(i));
                }
                cb.setTag(i);
    ...
    public List<CommentTag> getCheckedValues() {
        List<CommentTag> list = new ArrayList<CommentTag>();
        if (singleChoice && mLastCheckedPosition >= 0
                && mLastCheckedPosition < viewList.size()) {
            list.add(childValues.get(mLastCheckedPosition));
            return list;
        }
        for (int i = 0; i < viewList.size(); i++) {
            if (viewList.get(i).isChecked()) {
                list.add(childValues.get(i));
            }
        }
        return list;
    }

    public int append(CommentTag str) {
        CheckBox cb = getChild();
        cb.setText(str.getLabel_detail());
    ...
    public void addAll(List<CommentTag> list) {
        if (list != null && list.size() > 0) {
            for (CommentTag str : list) {
                append(str);
            }
        }
    }
    ...
    public boolean insert(int position, CommentTag str) {
        if (position < 0 || position > viewList.size()) {
            return false;
        }
        CheckBox cb = getChild();
