...
public class MessageActivity extends ToolbarActivity {
    ...
    protected void initDatas() {
        ...
        messageAdapter = new MessageAdapter(new RecyclerViewAdapter.AdapterListenerImpl<Massage>() {
            @Override
            public void onItemClick(RecyclerViewAdapter.ViewHolder<Massage> holder, Massage data) {
    ...
    private void getMessageList(final int page) {
        ...
        okHttpClient.post("/v1/system-message/massage-list", AsyncHttpPostFormData, new AsyncResponseHandler() {
            @Override
            ...
            public void onSuccess(AsyncHttpResponse response) {
                Log.i(TAG, "onSuccess:消息列表" + response.toString());
                Type type = new TypeToken<RspModel<List<Massage>>>() {
                }.getType();
                List<Massage> list = getAnalysis(response, type, "消息列表");
                if (list != null && !"[]".equals(list.toString())) {
                    messageAdapter.replace(list);
                    if(page==1){
                        messageAdapter.replace(list);
                    }else{
