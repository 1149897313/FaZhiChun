...
public class ExistOrderFragment extends Fragment {
    ...
    protected void initDatas() {
        ...
        mAllOrderAdapter = new AllOrderAdapter(new RecyclerViewAdapter.AdapterListenerImpl<OrderInfo>() {
            @Override
        ...
        mRecyclerView.setAdapter(mAllOrderAdapter);
    }
    ...
    private void geOrderList(final int page, String condition) {
        ...
        okHttpClient.post("/v1/order/list", AsyncHttpPostFormData, new AsyncResponseHandler() {
            ...
            public void onSuccess(AsyncHttpResponse response) {
                Log.i(TAG, "onSuccess:全部订单：" + response.toString());
                Type type = new TypeToken<RspModel<List<OrderInfo>>>() {
                }.getType();
                List<OrderInfo> order = getAnalysis(response, type, "全部订单");
                if (order != null && !"[]".equals(order.toString())) {
                    mLoadManager.showSuccessView();
                    if(page==1) {
                        mAllOrderAdapter.replace(order);
                    }else {
                        mAllOrderAdapter.add(order);
                    }
                } else {
                    if(page==1){
