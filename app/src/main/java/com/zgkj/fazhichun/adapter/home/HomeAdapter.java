...
public class HomeAdapter extends RecyclerViewAdapter<StoreListAndProductList> {
    ...
    private class HomeViewHolder extends RecyclerViewAdapter.ViewHolder<StoreListAndProductList> implements View.OnClickListener {
        ...
        @Override
        protected void onBind(final StoreListAndProductList data,int position) {
            mShopNameView.setText(data.getShop_name());
            mScoreView.setText(String.valueOf(data.getShop_service_score()) + "分");
            mDistanceView.setText(df.format(data.getDistance()/1000.00d)+"KM");
            mAddressView.setText(data.getAddress());
            Picasso.get().load("".equals(data.getShop_image()) ? mContext.getResources().getString(R.string.none_image_url) : data.getShop_image()).placeholder(R.drawable.none_img).transform(new CropSquareTransformation()).noFade().error(R.drawable.image_error)
                    .into(mShopImageView);
